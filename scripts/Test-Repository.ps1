...
foreach ($relativePath in $requiredPaths) {
    ...
    Assert-True -Condition (Test-Path -LiteralPath $absolutePath) -Message "Erforderliche Datei fehlt: $relativePath"
}
...
Assert-True -Condition $frontmatterMatch.Success -Message 'SKILL.md hat kein gültiges, minimales YAML-Frontmatter.'

$skillName = $frontmatterMatch.Groups[1].Value.Trim()
$description = $frontmatterMatch.Groups[2].Value.Trim()
Assert-True -Condition ($skillName -eq 'windows-pc-guru') -Message "Unerwarteter Skillname: $skillName"
Assert-True -Condition ($skillName -match '^[a-z0-9]+(?:-[a-z0-9]+)*$') -Message 'Der Skillname ist nicht standardkonform.'
Assert-True -Condition ($skillName.Length -le 64) -Message 'Der Skillname ist länger als 64 Zeichen.'
Assert-True -Condition ($description.Length -ge 80 -and $description.Length -le 1024) -Message 'Die Skillbeschreibung muss 80 bis 1024 Zeichen lang sein.'
Assert-True -Condition ($skillText -notmatch '(?i)\[?TODO|PLACEHOLDER') -Message 'SKILL.md enthält noch einen Platzhalter.'

$markdownLinks = [regex]::Matches($skillText, '\]\(([^)#]+\.md)(?:#[^)]+)?\)')
foreach ($link in $markdownLinks) {
    $target = Join-Path $skillRoot $link.Groups[1].Value
    Assert-True -Condition (Test-Path -LiteralPath $target) -Message "Defekter relativer Link in SKILL.md: $($link.Groups[1].Value)"
}
...
foreach ($markdownFile in $markdownFiles) {
    ...
    foreach ($relativeLink in $relativeLinks) {
        $decodedTarget = [uri]::UnescapeDataString($relativeLink.Groups[1].Value.Trim('<', '>'))
        $targetPath = Join-Path $markdownFile.DirectoryName $decodedTarget
        Assert-True -Condition (Test-Path -LiteralPath $targetPath) -Message "Defekter relativer Link in $($markdownFile.FullName): $decodedTarget"
    }
}

$openAiYamlPath = Join-Path $skillRoot 'agents\openai.yaml'
$openAiYaml = Get-Content -Raw -Encoding UTF8 -LiteralPath $openAiYamlPath
Assert-True -Condition ($openAiYaml -match 'display_name:\s*"Windows-PC-Guru"') -Message 'agents/openai.yaml enthält keinen passenden display_name.'
Assert-True -Condition ($openAiYaml -match '\$windows-pc-guru') -Message 'Der OpenAI-Standardprompt erwähnt den Skill nicht explizit.'
...
foreach ($powerShellFile in $powerShellFiles) {
    ...
    Assert-True -Condition ($parseErrors.Count -eq 0) -Message "PowerShell-Syntaxfehler in $($powerShellFile.FullName): $($parseErrors -join '; ')"
}
...
foreach ($runtimeScript in $runtimeScripts) {
    ...
    foreach ($pattern in $forbiddenRuntimePatterns) {
        Assert-True -Condition ($runtimeText -notmatch [regex]::Escape($pattern)) -Message "Verbotenes Laufzeitmuster '$pattern' in $($runtimeScript.Name)."
    }
}
...
if ($RunDiagnosticsSmokeTest) {
    ...
    Assert-True -Condition ($snapshot.SchemaVersion -eq '2.0') -Message 'Der Diagnose-Smoke-Test lieferte nicht Snapshot-Schema 2.0.'
    ...
    Assert-True -Condition ($baseline.SchemaVersion -eq '1.0') -Message 'Die Leistungs-Baseline lieferte ein unerwartetes Schema.'
    Assert-True -Condition ($baseline.PrivacyNotice -match 'Keine Benutzer') -Message 'Die Leistungs-Baseline enthält keinen Datenschutzhinweis.'
    Assert-True -Condition (-not $baseline.NetworkUsed) -Message 'Die Leistungs-Baseline meldet unerwarteten Netzwerkzugriff.'

    $localOnlyPolicy = & (Join-Path $repositoryRoot 'scripts\Test-LocalOnlyPolicy.ps1') -AsJson | ConvertFrom-Json
    Assert-True -Condition $localOnlyPolicy.Allowed -Message 'Die Lokal-only-Pruefung hat ein verbotenes Laufzeitmuster gefunden.'
    Assert-True -Condition (-not $localOnlyPolicy.NetworkUsed) -Message 'Die Lokal-only-Pruefung meldet unerwarteten Netzwerkzugriff.'

    $driverScript = Join-Path $skillRoot 'scripts\Test-DriverPackage.ps1'
    $driverResult = & $driverScript -LiteralPath $driverScript -AsJson | ConvertFrom-Json
    Assert-True -Condition ($driverResult.SHA256 -match '^[A-F0-9]{64}$') -Message 'Die Paketprüfung lieferte keinen gültigen SHA-256-Wert.'
    Assert-True -Condition (-not $driverResult.NetworkUsed) -Message 'Die Paketprüfung meldet unerwarteten Netzwerkzugriff.'
    Assert-True -Condition (-not $driverResult.InstalledAnything) -Message 'Die Paketprüfung meldet eine unerwartete Installation.'
}
